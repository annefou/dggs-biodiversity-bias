...
| 07 | `07_multigrid_quercus_suber.py` | Comprehensive comparison of *Q. suber* density on **eight grids**: lat-lon (cautionary baseline), HEALPix nside=64 (DGGS, sphere), **HEALPix-geo nside=64 (DGGS, WGS84 — GRID4EARTH path via authalic-sphere mapping)**, H3 res 3 (hexagonal DGGS), rHEALPix res 4 (DGGS, WGS84 cube-projected), Mollweide ~100 km (equal-area projection), EEA reference grid 100 km (LAEA Europe / EPSG:3035, INSPIRE / Habitats Directive standard), and ISEA3H res 8 (the system the Eco-ISEA3H paper advocates). Confirms that **all seven equal-area choices agree** on the apparent density pattern; lat-lon is the only one that distorts. |
